...
class PLAYERMAIN1 {
    public static void main(String args[]) {
        ...
        for (i = 0; i < m; i++) {
            BA[i] = new BATSMAN();
            System.out.println(
                    "Enter name,number of matches played,and number of runs scored by Batsman " + (i + 1) + ":");
            BA[i].name = ss.next();
        ...
        for (i = 0; i < n; i++) {
            BO[i] = new BOWLER();
            System.out
                    .println("Enter name,number of matches played,and number of runs given by Bowler " + (i + 1) + ":");
            BO[i].name = ss.next();
